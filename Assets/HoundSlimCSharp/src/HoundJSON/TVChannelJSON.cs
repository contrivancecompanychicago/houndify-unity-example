...
public class TVChannelJSON : JSONBase
  {
    ...
    public enum TypeUploadOperationKnownValues
      {
        UploadOperation_Add,
        UploadOperation_Remove,
        UploadOperation__none
      };
    public struct TypeUploadOperation
      {
        public bool in_known_list;
        public string string_value;
        public TypeUploadOperationKnownValues list_value;
      };

    public static TypeUploadOperationKnownValues  stringToUploadOperation(string chars)
      {
        switch (chars[0])
          {
            case 'A':
                if ((String.Compare(chars, 1, "dd", 0, 2, false) == 0) && (chars.Length == 3))
                    return TypeUploadOperationKnownValues.UploadOperation_Add;
                break;
            case 'R':
                if ((String.Compare(chars, 1, "emove", 0, 5, false) == 0) && (chars.Length == 6))
                    return TypeUploadOperationKnownValues.UploadOperation_Remove;
                break;
            default:
                break;
          }
        return TypeUploadOperationKnownValues.UploadOperation__none;
      }

    public static string  stringFromUploadOperation(TypeUploadOperationKnownValues the_enum)
      {
        switch (the_enum)
          {
            case TypeUploadOperationKnownValues.UploadOperation_Add:
                return "Add";
            case TypeUploadOperationKnownValues.UploadOperation_Remove:
                return "Remove";
            default:
                Debug.Assert(false);
                return null;
    ...
    private bool flagHasUploadOperation;
    private TypeUploadOperation storeUploadOperation;
    ...
    private void  fromJSONUploadOperation(JSONValue json_value, bool ignore_extras)
      {
        Debug.Assert(json_value != null);
        JSONStringValue json_string = json_value.string_value();
        if (json_string == null)
            throw new Exception("The value for field UploadOperation of TVChannelJSON is not a string.");
        TypeUploadOperation the_open_enum = new TypeUploadOperation();
        switch (json_string.getData()[0])
          {
            case 'A':
                if ((String.Compare(json_string.getData(), 1, "dd", 0, 2, false) == 0) && (json_string.getData().Length == 3))
                      {
                        the_open_enum.in_known_list = true;
                        the_open_enum.list_value = TypeUploadOperationKnownValues.UploadOperation_Add;
                        goto open_enum_is_done;
                      }
                break;
            case 'R':
                if ((String.Compare(json_string.getData(), 1, "emove", 0, 5, false) == 0) && (json_string.getData().Length == 6))
                      {
                        the_open_enum.in_known_list = true;
                        the_open_enum.list_value = TypeUploadOperationKnownValues.UploadOperation_Remove;
                        goto open_enum_is_done;
                      }
                break;
            default:
        ...
        the_open_enum.string_value = json_string.getData();
      open_enum_is_done:;
        setUploadOperation(the_open_enum);
      }
    ...
    public bool  hasUploadOperation()
      {
        return flagHasUploadOperation;
      }

    public TypeUploadOperation  getUploadOperation()
      {
        Debug.Assert(flagHasUploadOperation);
        return storeUploadOperation;
      }

    public string  getUploadOperationAsString()
      {
        TypeUploadOperation result = getUploadOperation();
        if (result.in_known_list)
            return stringFromUploadOperation(result.list_value);
        else
            return result.string_value;
      }
    ...
    public void setUploadOperation(TypeUploadOperation new_value)
      {
        flagHasUploadOperation = true;
        storeUploadOperation = new_value;
      }
    public void setUploadOperation(string chars)
      {
        TypeUploadOperationKnownValues known = stringToUploadOperation(chars);
        TypeUploadOperation new_value = new TypeUploadOperation();
        if (known == TypeUploadOperationKnownValues.UploadOperation__none)
          {
            new_value.in_known_list = false;
            new_value.string_value = chars;
          }
        ...
        setUploadOperation(new_value);
      }
    public void setUploadOperation(TypeUploadOperationKnownValues new_value)
      {
        TypeUploadOperation new_full_value = new TypeUploadOperation();
        Debug.Assert(new_value != TypeUploadOperationKnownValues.UploadOperation__none);
        new_full_value.in_known_list = true;
        new_full_value.list_value = new_value;
        setUploadOperation(new_full_value);
      }
    public void unsetUploadOperation()
      {
        flagHasUploadOperation = false;
      }
    ...
    public virtual void write_fields_as_json(JSONHandler handler, bool partial_allowed)
      {
        ...
        if (flagHasUploadOperation)
          {
            handler.start_pair("UploadOperation");
            if (storeUploadOperation.in_known_list)
              {
                switch (storeUploadOperation.list_value)
                  {
                    case TypeUploadOperationKnownValues.UploadOperation_Add:
              ...
              {
                        handler.string_value(storeUploadOperation.string_value);
              }
          }
      }
...
//@@@        Debug.Assert(!have_value);
    ...
    private class FieldHoldingArrayGeneratorDefinition : JSONArrayGenerator
  {
    ...
    private abstract class FieldGeneratorUploadOperation : JSONStringGenerator
          {
            protected FieldGeneratorUploadOperation(string what)
              {
                set_what(what);
              }
            protected FieldGeneratorUploadOperation()
              {
              }
            protected override void handle_result(string result)
              {
                TypeUploadOperationKnownValues known = stringToUploadOperation(result);
                TypeUploadOperation new_value = new TypeUploadOperation();
                if (known == TypeUploadOperationKnownValues.UploadOperation__none)
                  {
                    new_value.in_known_list = false;
            ...
            protected abstract void handle_result(TypeUploadOperation result);
          };
    private class FieldHoldingGeneratorUploadOperation : FieldGeneratorUploadOperation
  {
    protected override void handle_result(TypeUploadOperation result)
      {
//@@@        Debug.Assert(!have_value);
        have_value = true;
        value = result;
    ...
    public FieldHoldingGeneratorUploadOperation(String what)
      {
        have_value = false;
        base.set_what(what);
      }
    ...
    public bool have_value;
    public TypeUploadOperation value;
  };
    private class FieldHoldingArrayGeneratorUploadOperation : JSONArrayGenerator
  {
    protected class ElementHandler : FieldGeneratorUploadOperation
      {
        private FieldHoldingArrayGeneratorUploadOperation top;
        ...
        public ElementHandler(FieldHoldingArrayGeneratorUploadOperation init_top)
          {
            top = init_top;
          }
      };
    ...
    protected virtual void handle_result(List<TypeUploadOperation> result)
      {
      }

    public FieldHoldingArrayGeneratorUploadOperation(string what)
      {
        element_handler = new ElementHandler(this);
        have_value = false;
        value = new List<TypeUploadOperation>();
        base.set_what(what);
      }
    public FieldHoldingArrayGeneratorUploadOperation()
      {
        element_handler = new ElementHandler(this);
        have_value = false;
        value = new List<TypeUploadOperation>();
    ...
    public bool have_value;
    public List<TypeUploadOperation> value;
  };
        private FieldHoldingGeneratorUploadOperation fieldGeneratorUploadOperation;
        ...
        protected void finish(TVChannelJSON result)
          {
            ...
            if (fieldGeneratorUploadOperation.have_value)
              {
                result.setUploadOperation(fieldGeneratorUploadOperation.value);
                fieldGeneratorUploadOperation.have_value = false;
              }
          }
            ...
            switch (field_name[0])
              {
                    ...
                    switch (field_name[1])
                      {
                        case 'a':
                            ...
                            break;
                        case 'h':
                            if (String.Compare(field_name, 2, "annelN", 0, 6, false) == 0)
                              {
                                switch (field_name[8])
                                  {
                                    case 'a':
                ...
                case 'S':
                    ...
                    break;
                case 'U':
                    if ((String.Compare(field_name, 1, "ploadOperation", 0, 14, false) == 0) && (field_name.Length == 15))
                        return fieldGeneratorUploadOperation;
                    break;
                default:
                    break;
              }
        ...
        public Generator(bool ignore_extras)
          {
            ...
            fieldGeneratorUploadOperation = new FieldHoldingGeneratorUploadOperation("field \"UploadOperation\" of the TVChannel class");
        ...
        public override void reset()
          {
            ...
            fieldGeneratorUploadOperation.reset();
            base.reset();
