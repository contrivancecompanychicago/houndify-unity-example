    ...
    private void  fromJSONChoiceList(JSONValue json_value, bool ignore_extras)
      {
        ...
        Debug.Assert(vector_ChoiceList1.Count >= 2);
        initChoiceList();
        for (int num2 = 0; num2 < vector_ChoiceList1.Count; ++num2)
    ...
    public void initChoiceList()
      {
        if (flagHasChoiceList)
          {
            for (int num3 = 0; num3 < storeChoiceList.Count; ++num3)
              {
              }
          }
        flagHasChoiceList = true;
    ...
    public void unsetChoiceList()
      {
        if (flagHasChoiceList)
          {
            for (int num4 = 0; num4 < storeChoiceList.Count; ++num4)
              {
              }
          }
        flagHasChoiceList = false;
        ...
        public Generator(bool ignore_extras) : base(ignore_extras)
          {
            ...
            fieldGeneratorNativeData = new TypeNativeDataJSON.HoldingGenerator("field \"NativeData\" of the DisambiguateCommand class", ignore_extras);
            unknownFieldGenerator = new UnknownFieldGenerator(ignore_extras);
        ...
        public Generator() : base(false)
          {
            ...
            fieldGeneratorNativeData = new TypeNativeDataJSON.HoldingGenerator("field \"NativeData\" of the DisambiguateCommand class", false);
            unknownFieldGenerator = new UnknownFieldGenerator(false);
            keyGenerator = new JSONHoldingStringGenerator("key field \"DisambiguateCommandKind\" of the DisambiguateCommand class");
            set_what("the DisambiguateCommand class");
          }
...
//@@@            Debug.Assert(!have_value);
    ...
    public new class HoldingArrayGenerator : JSONArrayGenerator
  {
    ...
    switch (key[0])
      {
        case 'C':
            if (String.Compare(key, 1, "all", 0, 3, false) == 0)
              {
                switch (key[4])
                  {
                    case 'O':
