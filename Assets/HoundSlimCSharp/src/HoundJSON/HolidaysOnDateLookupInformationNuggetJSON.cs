    ...
    private JSONValue  extraHolidaysToJSON()
      {
        ...
        for (int num1 = 0; num1 < storeHolidays.Count; ++num1)
          {
            JSONStringValue generated_string_Holidays = new JSONStringValue(storeHolidays[num1]);
    ...
    private void  fromJSONHolidays(JSONValue json_value, bool ignore_extras)
      {
        ...
        for (int num1 = 0; num1 < vector_Holidays1.Count; ++num1)
            appendHolidays(vector_Holidays1[num1]);
        for (int num1 = 0; num1 < vector_Holidays1.Count; ++num1)
          {
          }
      }
        ...
        switch (field_name[0])
          {
            case 'D':
                ...
                break;
            case 'H':
                if ((String.Compare(field_name, 1, "olidays", 0, 7, false) == 0) && (field_name.Length == 8))
        ...
        switch (key[0])
          {
            case 'D':
                ...
                break;
            case 'H':
                if ((String.Compare(key, 1, "olidays", 0, 7, false) == 0) && (key.Length == 8))
                    {
