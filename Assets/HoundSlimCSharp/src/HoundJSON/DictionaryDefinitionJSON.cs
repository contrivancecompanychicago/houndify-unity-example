...
public class DictionaryDefinitionJSON : JSONBase
  {
    ...
    private List< DictionaryPartOfSpeechJSON  > storePartOfSpeech;
    ...
    public DictionaryDefinitionJSON()
      {
        ...
        storePartOfSpeech = new List< DictionaryPartOfSpeechJSON  >();
    ...
    public int  countOfPartOfSpeech()
      {
        Debug.Assert(flagHasPartOfSpeech);
        return storePartOfSpeech.Count;
      }

    public DictionaryPartOfSpeechJSON   elementOfPartOfSpeech(int element_num)
      {
        Debug.Assert(flagHasPartOfSpeech);
        return storePartOfSpeech[element_num];
      }

    public List< DictionaryPartOfSpeechJSON  >  getPartOfSpeech()
      {
        Debug.Assert(flagHasPartOfSpeech);
    ...
    public void initPartOfSpeech()
      {
        if (flagHasPartOfSpeech)
          {
            for (int num1 = 0; num1 < storePartOfSpeech.Count; ++num1)
              {
              }
          }
    ...
    public void appendPartOfSpeech(DictionaryPartOfSpeechJSON.TypeValue new_value)
      {
        appendPartOfSpeech(new DictionaryPartOfSpeechJSON(new_value));
      }
    ...
    public void unsetPartOfSpeech()
      {
        if (flagHasPartOfSpeech)
          {
            for (int num2 = 0; num2 < storePartOfSpeech.Count; ++num2)
              {
              }
          }
        flagHasPartOfSpeech = false;
        ...
        if (flagHasPartOfSpeech)
          {
            ...
            for (int num1 = 0; num1 < storePartOfSpeech.Count; ++num1)
              {
                if (partial_allowed)
                    storePartOfSpeech[num1].write_partial_as_json(handler);
                else
                    storePartOfSpeech[num1].write_as_json(handler);
              }
        ...
        if (flagHasExamples)
          {
            ...
            handler.start_array();
            for (int num2 = 0; num2 < storeExamples.Count; ++num2)
              {
                handler.string_value(storeExamples[num2]);
              }
            handler.finish_array();
          }
      }
    ...
    public abstract class Generator : JSONObjectGenerator
      {
        ...
        private DictionaryPartOfSpeechJSON.HoldingArrayGenerator fieldGeneratorPartOfSpeech;
        ...
        protected void finish(DictionaryDefinitionJSON result)
          {
            ...
            if (fieldGeneratorPartOfSpeech.have_value)
              {
                result.initPartOfSpeech();
                int count = fieldGeneratorPartOfSpeech.value.Count;
                for (int num = 0; num < count; ++num)
                  {
                    result.appendPartOfSpeech(fieldGeneratorPartOfSpeech.value[num]);
                  }
                fieldGeneratorPartOfSpeech.value.Clear();
        ...
        public Generator(bool ignore_extras)
          {
            ...
            fieldGeneratorPartOfSpeech = new DictionaryPartOfSpeechJSON.HoldingArrayGenerator("field \"PartOfSpeech\" of the DictionaryDefinition class", ignore_extras);
