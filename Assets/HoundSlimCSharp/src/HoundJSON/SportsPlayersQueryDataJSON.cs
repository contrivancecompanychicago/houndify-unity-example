    ...
    private void  fromJSONPlayersIndices(JSONValue json_value, bool ignore_extras)
      {
        ...
        initPlayersIndices();
        for (int num2 = 0; num2 < vector_PlayersIndices1.Count; ++num2)
            appendPlayersIndices(vector_PlayersIndices1[num2]);
        ...
        if (flagHasPlayersIndices)
          {
            ...
            handler.start_array();
            for (int num2 = 0; num2 < storePlayersIndices.Count; ++num2)
              {
                handler.number_value(storePlayersIndices[num2]);
              }
            handler.finish_array();
          }
        if (flagHasError)
                                ...
                                switch (field_name[6])
                                  {
                                    case 'I':
                                        ...
                                        break;
                                    case 's':
                                        if ((String.Compare(field_name, 7, "Indices", 0, 7, false) == 0) && (field_name.Length == 14))
