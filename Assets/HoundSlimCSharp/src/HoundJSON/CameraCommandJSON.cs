...
public class CameraCommandJSON : DeviceControlCommandJSON
  {
    ...
    public static TypeCommandType  stringToCommandType(string chars)
      {
        if ((String.Compare(chars, 0, "TakeAPicture", 0, 12, false) == 0) && (chars.Length == 12))
    ...
    private void  fromJSONCommandType(JSONValue json_value, bool ignore_extras)
      {
        ...
        if (!(json_string.getData().Equals("TakeAPicture")))
            throw new Exception("The value for field CommandType of CameraCommandJSON is not `TakeAPicture'.");
        setCommandType();
      }
