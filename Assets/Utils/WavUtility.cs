...
public class WavUtility
{
    ...
    public static byte[] FromAudioClipWithoutHeader(AudioClip audioClip)
    {
        MemoryStream stream = new MemoryStream();
        // get bit depth
        UInt16 bitDepth = 16; //BitDepth (audioClip);
        int fileSize = audioClip.samples * BlockSize_16Bit; // BlockSize (bitDepth)
        WriteFileData(ref stream, audioClip, bitDepth);
        byte[] bytes = stream.ToArray();
        stream.Dispose();
        return bytes;
    ...
    //static byte[] WriteHeader(AudioClip clip)
    //{
